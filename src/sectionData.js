const sectionData = [
    {
        title: "bread",
        text: "Good bread is made with good ingredients. Great bread is made with intention.",
        images: [
        {
            path: '/images/Bread1.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread2.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread3.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread4.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread5.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread6.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread7.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread8.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread9.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Bread10.jpg',
            alt: 'fresh-baked croissants'
        }
        ]
    },
    {
        title: "sweet",
        text: "We believe a sweet treat is one of life's great pleasures. After you try one of these pastries, we think you'll agree.",
        images: [
        {
            path: '/images/Sweet1.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet2.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet3.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet4.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet5.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet6.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet7.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet8.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet9.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet10.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Sweet11.jpg',
            alt: 'fresh-baked croissants'
        }
        ]
    },
    {
        title: "savory",
        text: "Rich, buttery, flavorful, tender, flaky. Just some of the words we would use to describe our savory pastries.",
        images: [
        {
            path: '/images/Savory1.png',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Savory2.png',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Savory3.png',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Savory4.png',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Savory5.png',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Savory6.png',
            alt: 'fresh-baked croissants'
        }
        ]
    },
    {
        title: "story",
        content_1: '"We consider Fahrenheit Bakery more than just a building. For us it\'s like a home."',
        content_2: "The idea was born a decade ago. Maria lived in San Francisco, and was training at a baking school when she met Matthew. He worked while she spent hours each day studying and mastering different American baking techniques. In 2013, they got married in Maria's hometown in Cyprus! It was on this trip that they decided they would eventually move to Cyprus permanently to open their own American-style bakery.",
        content_3: "Fast-forward through many years of hard work, and in January 2022 they opened Fahrenheit Bakery in Larnaca! The people of Larnaca welcomed them with open arms and eager stomachs. The bakery is a link between their lives in San Francisco and Cyprus. When you stop by, you'll notice it is also a family affair – Maria's parents can be seen helping out on most days. Above all, they hope that everyone who stops by the bakery feels welcomed as part of the Fahrenheit family.",
        images: [
        {
            path: '/images/Story1.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Story2.png',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Story3.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Story4.jpg',
            alt: 'fresh-baked croissants'
        },
        {
            path: '/images/Story5.jpg',
            alt: 'fresh-baked croissants'
        }
        ]
    }
]

export default sectionData